        ...
        if (err) {
          ...
          peripheral.discoverAllServicesAndCharacteristics(function (err, services, characteristics) {
            if (err) {
              ...
              writeChunks(characteristics[4], buffer, function (err) {
                cb(err)
              })
            }
          })
...
var writeChunks = function (characteristic, buffer, cb) {
  ...
  // Throttle writing, causes problems with windows
  function sendNext(i) {    
    if(i < 291) { // Send the dithered 1 bit per pixel 176x264 image data
      characteristic.write(buffer.slice(i * 20, i * 20 + 20), true, function (err) {
        if (err) cb(err);
        else setTimeout(sendNext, 10, i+1);

      });     
      
    } else if(i == 291) { // End writing by sending 'd'
  ...
  // Start writing by sending char 't'
